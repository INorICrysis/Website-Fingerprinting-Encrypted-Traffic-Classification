...
# 日志设置
LOG_DIR = "./log/train_rf_mlp"
os.makedirs(LOG_DIR, exist_ok=True)
LOG_FILE = os.path.join(LOG_DIR, "train_rf_mlp.txt")
MODEL_FILE = os.path.join(LOG_DIR, "rf_mlp_best_model.pth")
...
rf_train_proba = rf.predict_proba(X_train_scaled)
rf_test_proba = rf.predict_proba(X_test_scaled)

# 拼接原始数据 & RF概率
X_train_aug = np.hstack([X_train_scaled, rf_train_proba])
X_test_aug = np.hstack([X_test_scaled, rf_test_proba])
...
# DataLoader & 模型构建
train_loader, test_loader = get_dataloader(X_train_aug, y_train, X_test_aug, y_test, batch_size=128)

mlp = MLP(
    in_dim=X_train_aug.shape[1],
